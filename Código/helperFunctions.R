...
# Description: helper functions to carry out microarray analysis with limma, 
# affy, etc
# Date: 03/06/20

## annotate expression matrix from probes names
annotateMatrix <- function(probesNames, dbObject, columns) {
  dfAnnotated <- data.frame(sapply(columns, function(column) {
                                              mapIds(dbObject,
                                                     keys = probesNames, 
                                                     column = column, 
                                                     keytype = "PROBEID")
...
## search a gene by symbol in exprSet object
searchGeneExprSet <- function(gene, objExprSet, genesMetadata, column = "SYMBOL") {
  matches <- which(genesMetadata[, column] == gene)
  if (length(matches) == 0)
    stop("No matches for ", gene)
  
  exprSet <- exprs(objExprSet)[rownames(genesMetadata[matches,]), , drop = FALSE]
  if (dim(exprSet)[1] == 0)
    stop("No matches for ", gene)
  else if (dim(exprSet)[1] > 1)
    message("More than one match for ", gene, " in exprSet object.")
...
## plot PCA
plotPCA <- function(pcaObject, col.points, shape.points, palette, legend.col, 
                    legend.shape, shapes = c(21, 24), point.size = 3, 
                    title = ""){
  variance <- round(get_eigenvalue(pcaObject)[c(1, 2), 2], 1)
  
  p <- ggplot(data.frame(pcaObject[["x"]]), 
              aes(x = PC1, y = PC2, fill = col.points, shape = shape.points)) +
    geom_point(size = point.size, color = "black") +
  # + scale_color_manual(name = legend.col, values = palette) +
    scale_fill_manual(name = legend.col, values = palette) + 
    scale_shape_manual(name = legend.shape, values = shapes) + 
    xlab(paste0("PC1 (", variance[1], "%)")) + ylab(paste0("PC2 (", variance[2], "%)")) + 
...
extractInfoVolcano <- function(MArrayLMObj, coef = 1, style = "p-value"){
  x <- as.matrix(MArrayLMObj$coefficients)[, coef]
  if (style == "p-value")
    y <- as.matrix(-log10(MArrayLMObj$p.value)[, coef])
  else
    y <- as.matrix(MArrayLMObj$lods)[, coef]
...
## customize volcano plot
volcanoplotCustom <- function(MArrayLMObj, coef = 1, style = "p-value", xLim, 
                              yLim, colors = c("#9e2121", "#2d7fcc", "#4a5054"),
                              legend = TRUE, pos.legend = "topleft", 
                              legend.text = NULL, line.color = "black",
                              highlight = TRUE, numLables = 5){
  
  if (!is(MArrayLMObj, "MArrayLM")) 
    stop("fit must be an MArrayLM")
  
  data <- extractInfoVolcano(MArrayLMObj = MArrayLMObj, coef = coef, 
                             style = style)
  
  upGenes <- subset(data, x > xLim & y > yLim)
  downGenes <- subset(data, x < -xLim & y > yLim)
  nonGenes <- data[!(rownames(data) %in% rownames(upGenes)) &
                   !(rownames(data) %in% rownames(downGenes)), ]
  
  points(upGenes, col = colors[1], pch = 20, cex = 0.4)
  points(downGenes, col = colors[2], pch = 20, cex = 0.4)
  points(nonGenes, col = colors[3], pch = 20, cex = 0.4)
  ...
  if (highlight) {
    labelsUp <- head(with(upGenes, upGenes[order(-y, -x),], n = numLabels))
    geneNamesUp <- match(rownames(labelsUp), rownames(MArrayLMObj$genes))
    maptools::pointLabel(labelsUp$x, labelsUp$y, 
                         MArrayLMObj$genes$ALIAS[geneNamesUp], cex = 0.7)
    
    labelsDown <- head(with(downGenes, downGenes[order(-y, -x),], n = numLabels))
    geneNamesDown <- match(rownames(labelsDown), rownames(MArrayLMObj$genes))
    maptools::pointLabel(labelsDown$x, labelsDown$y, 
                         MArrayLMObj$genes$ALIAS[geneNamesDown], cex = 0.7)
  }
}


# MAplot for ExprSet objetcs using ma.plot functions from affy package
MAplotExprSet <- function(objExprs, array, group = FALSE, title = NULL, pch = ".", ...) {
  
  x <- log2(exprs(objExprs))
  names_samples <- colnames(exprs(objExprs))
  ...
  for (i in array) {
    ma.plot(A[, i], M[, i], xlab = "A", ylab = "M", 
            pch = pch, cex = 1, main = paste(names_samples[i], "vs pseudo-median (normalized)"),
            ...)
  }
}
...
## Venn diagram customized
## from https://gist.github.com/mevers/9c846e6293db44dd37695c46b8f2b6a2
...
## Venn diagram customized
vennDiagramGenes <- function(genes1, genes2, names, colors, title = "") {
  
  gene_union <- union(genes1$SYMBOL, 
                      genes2$SYMBOL)
  genes1_array <- array(0, dim = c(length(gene_union)))
  genes2_array <- array(0, dim = c(length(gene_union)))
  
  genes1_array[match(genes1$SYMBOL, gene_union)] = 1
  genes2_array[match(genes2$SYMBOL, gene_union)] = 1
  
  venncounts_all <- cbind(genes1_array, genes2_array)
  venncounts <- vennCounts(venncounts_all)
  
  my.vennDiagram(venncounts,
                 names = names,
                 circle.col = colors)
...
## write cls to GSEA
write.GSEA.cond <- function(objExprss, dfMetadata, filePath) {
  ...
  cat(paste(ncol(DF), length(unique(dfMetadata$Condition)), 1,
            sep = "\t"), file = filePath, sep = "\n")
  cat(paste("#", paste(unique(dfMetadata.HPBALL$Condition), collapse = "\t"), 
            sep = "\t"), sep = "\n", 
      file = filePath, append = TRUE)
  cat(paste(dfMetadata$Condition, collapse = "\t"), file = filePath, sep = "\n", 
      append = TRUE)
}


## write chip to GSEA
write.GSEA.conver <- function(objExprss, allGenes, filePath) {
  write.table(data.frame("Probe_Set_ID" = rownames(exprs(objExprss)),
                         "Gene Symbol" = allGenes$SYMBOL,
                         "Gene Title" = allGenes$ALIAS), 
              file = filePath, sep = "\t", row.names = FALSE, quote = FALSE,
              col.names = c("Probe Set ID", "Gene Symbol", "Gene Title"))
}
